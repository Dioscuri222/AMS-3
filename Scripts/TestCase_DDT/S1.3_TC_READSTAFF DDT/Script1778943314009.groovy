...
// 1. Log In Admin (Gunakan akun valid)
WebUI.openBrowser('')
...
if (testType == 'SEARCH') {
    // Jika tipenya SEARCH, isi kolom pencarian dengan keyword dari CSV
    WebUI.sendKeys(findTestObject('Page_Attendance Management System/Staff_Special_Zidan/input_Search_Staff'), searchKeyword)

    // Berikan jeda 1 detik agar fitur pencarian selesai memfilter data di tabel
    WebUI.delay(1)
}

// 4. Verifikasi Keberadaan Data di Tabel (Berlaku untuk Skenario READ & SEARCH)
...
WebUI.waitForElementVisible(findTestObject('Page_Attendance Management System/Staff_Special_Zidan/td_Pagi_Check'), 5)
